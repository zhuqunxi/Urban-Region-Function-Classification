...
def ceshi(classifiers, X, y, X_T):
    ...
    for train_index, test_index in sss.split(X, y):
        ...
        for clf in classifiers:
            ...
            np.save('./feature/prob_val_XG_cnt_val_%d.npy' % cnt_val, test_pred_prob)
            np.save('./feature/prob_test_XG_cnt_val_%d.npy' % cnt_val, Res_prob)
            if name in acc_dict:
                acc_dict[name] += acc
...
# X = get_train_test_data(files_list=train_files_list, n_dim=n_train)
# X_T = get_train_test_data(files_list=test_files_list, n_dim=100000)
X = get_train_test_data(files_list=['train_all_RF_4866.npy'], n_dim=n_train)
X_T = get_train_test_data(files_list=['test_all_RF_4866.npy'], n_dim=100000)
...
XG_LABEL_pro, val_pro = ceshi(classifiers, X, y, X_T)
