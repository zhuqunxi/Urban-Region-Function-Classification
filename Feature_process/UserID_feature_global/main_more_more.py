...
file_num_each_job_train = 440000
...
def get_statistic_variable(tmp):
    ...
    if len(tmp) > 0:
        ...
        return list(np.percentile(tmp, [10, 90])) + [tmp.max(), tmp.min()]
    else:
...
if __name__ == '__main__':
    ...
    global_local = 'global'
    train_data_name = 'train_X_UserID_normal_%s_%s_more_more.npy' % (global_local, 'feature')
    test_data_name = 'test_X_UserID_normal_%s_%s_more_more.npy' % (global_local, 'feature')
